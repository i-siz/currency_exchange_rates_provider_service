...
/**
 * Exchange rate provider using second mock service.
...
public class MockProvider2 implements ExchangeRateProvider {
    ...
    private static final String PROVIDER_NAME = "mock-service-2";

    @Value("${exchange.providers.mock2.url:http://localhost:8082}")
    private String baseUrl;

    @Value("${exchange.providers.mock2.enabled:true}")
    private boolean enabled;

    private final RestTemplate restTemplate;

    public MockProvider2(RestTemplate restTemplate) {
    ...
    public Map<String, ExchangeRateDto> fetchExchangeRates(String baseCurrency) {
        if (!enabled) {
            log.debug("Mock provider 2 is disabled");
        ...
        try {
            ...
            log.debug("Fetching rates from mock-service-2 for base currency: {}", baseCurrency);
            MockServiceResponse response = restTemplate.getForObject(url, MockServiceResponse.class);

            if (response == null) {
                if (log.isErrorEnabled()) {
                    log.error("Mock service 2 returned null response");
                }
            ...
            if (log.isErrorEnabled()) {
                log.error("Error fetching rates from mock-service-2: {}", ex.getMessage(), ex);
            }
